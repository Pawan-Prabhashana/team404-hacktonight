...
import { listBudgetsForUser, listUserAccountsForAnalytics } from '@/server/repositories/smart-spend-repository'
import { listUserTransactionsForAnalytics } from '@/server/repositories/smart-spend-repository'
...
export async function simulateFinancialScenario(input: {
  ...
  const { userId, scenarioType, amountMinorUnits, categorySlug, accountId } = input
  ...
  if (scenarioType === 'purchase' || scenarioType === 'bill_payment' || scenarioType === 'transfer') {
    projectedBalance = accountBalance - amountMinorUnits

    if (projectedBalance < 0) {
      warnings.push(`This would overdraft your account by ${formatCurrency(Math.abs(projectedBalance))}.`)
      recommendations.push('Choose a lower amount or use a different account with sufficient funds.')
    } else if (projectedBalance < amountMinorUnits) {
      warnings.push(`After this, your account balance will be less than the transaction amount — consider if you can afford follow-on costs.`)
    }

    if (budget) {
      const newCategoryTotal = categorySpend + amountMinorUnits
      const usedPct          = Math.round((newCategoryTotal / budget.amountMinorUnits) * 100)
      if (usedPct > 100) {
        warnings.push(`This would put your ${categorySlug} spending at ${usedPct}% of your ${formatCurrency(budget.amountMinorUnits)} budget.`)
        recommendations.push(`You are already at ${Math.round((categorySpend / budget.amountMinorUnits) * 100)}% of your ${categorySlug} budget. Consider reducing this purchase.`)
      } else if (usedPct >= 80) {
        warnings.push(`After this you will be at ${usedPct}% of your ${categorySlug} budget.`)
      }
    }
  } else if (scenarioType === 'saving') {
    projectedBalance = accountBalance - amountMinorUnits
    if (projectedBalance < 0) {
      warnings.push(`You don't have enough balance to save ${formatCurrency(amountMinorUnits)}.`)
    } else {
      recommendations.push(`Saving ${formatCurrency(amountMinorUnits)} would boost your financial health score.`)
    }
  }

  const balanceDrop     = accountBalance - projectedBalance
  const balanceDropPct  = accountBalance > 0 ? (balanceDrop / accountBalance) * 100 : 100

  let impactLevel: 'low' | 'medium' | 'high' = 'low'
  if (balanceDropPct > 30 || projectedBalance < 0) impactLevel = 'high'
  else if (balanceDropPct > 10)                     impactLevel = 'medium'

  if (projectedBalance >= 0 && projectedBalance < amountMinorUnits * 2 && scenarioType !== 'saving') {
    recommendations.push('Consider delaying this purchase until your next income cycle.')
  }

  if (warnings.length === 0 && impactLevel === 'low') {
    recommendations.push('This transaction appears affordable based on your current balance.')
  }
