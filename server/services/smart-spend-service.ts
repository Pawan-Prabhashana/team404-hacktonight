...
function detectRecurring(
  ...
  for (const [key, g] of groups.entries()) {
    ...
    const avg = Math.round(
      g.amounts.reduce((a, b) => a + b, 0) / g.amounts.length
    )
    const confidence = key.startsWith('bill:')
      ? 0.95
      : Math.min(0.5 + g.amounts.length * 0.15, 0.9)
...
function computeHealthScore(params: {
  ...
  const allBudgetsSafe =
    params.categoriesOver === 0 && params.categoriesWatch === 0
  ...
  if (
    params.budgetDailyLimit > 0 &&
    params.avgDailySpend > params.budgetDailyLimit
  )
    score -= 5
...
export async function getSmartSpendSummary(input: {
  ...
  const budgetMap = new Map(
    budgets.map((b) => [b.categorySlug, b.amountMinorUnits])
  )
  ...
  const categories: CategoryBreakdownItem[] = Array.from(catMap.entries())
    ...
    .map(([slug, amount]) => {
      ...
      const pct =
        monthlySpend > 0 ? Math.round((amount / monthlySpend) * 100) : 0
      const budget = budgetMap.get(slug)
      const usedPct =
        budget && budget > 0 ? Math.round((amount / budget) * 100) : undefined
      ...
      return {
        ...
        budgetDisplay:
          budget !== undefined ? formatCurrency(budget) : undefined,
  ...
  if (budgets.length === 0 && monthlySpend > 0) {
    savingsPotential = Math.round(monthlySpend * 0.1)
  }
  ...
  const dailyBudgetLimit =
    totalBudget > 0 ? Math.round(totalBudget / daysInMonth()) : 0
  ...
  if (recurring.length > 0) {
    insights.push({
      ...
      message: `Regular charges detected: ${recurring
        .slice(0, 3)
        .map((r) => r.name)
        .join(', ')}.`
    })
  ...
  const upcomingRecurring = recurring.reduce(
    (s, r) => s + r.averageAmountMinorUnits,
    0
  )
  ...
  if (projectedEndBalance < 0) {
    forecastWarning =
      'Your projected month-end balance is negative. Consider reducing discretionary spending.'
  } else if (projectedEndBalance < avgDailySpend * 7) {
    forecastWarning =
      'Your projected month-end balance is low. Review upcoming recurring payments.'
  }
  ...
  return {
    ...
    forecast: {
      ...
      projectedMonthEndBalanceDisplay: formatCurrency(
        Math.max(0, projectedEndBalance)
      ),
